...
const stats = [
  { label: 'Utilisateurs Actifs', val: '124.5k', trend: '+15%', icon: Users, color: 'text-indigo-400', bg: 'bg-indigo-500/10 border-indigo-500/20' },
...
export default function SectionAccueil() {
  return (
    <div className="space-y-6">
      {/* Hero SaaS Control */}
      <div className="bg-gradient-to-br from-[#0B0F19] via-[#101728] to-[#1e1b4b] border border-indigo-500/20 rounded-3xl p-6 lg:p-8 relative overflow-hidden">
        <div className="absolute right-0 top-0 w-96 h-96 bg-indigo-500/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none"></div>
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-6 relative z-10">
          ...
          <div className="flex gap-3">
            ...
            <button className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-5 py-2.5 rounded-xl transition-colors text-sm shadow-lg shadow-indigo-500/20">
              Générer Rapport
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#101728] border border-slate-800/80 rounded-2xl p-6 lg:col-span-2">
          <div className="flex items-center justify-between mb-8">
            <h3 className="font-bold text-white flex items-center gap-2"><BarChart3 className="w-4 h-4 text-indigo-400" /> Croissance Utilisateurs & Volume Transactions (YTD)</h3>
            <div className="flex gap-2">
              <span className="text-xs font-semibold text-slate-400 flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-indigo-500"></span> Utilisateurs</span>
            ...
            {[30, 45, 40, 60, 55, 70, 65, 85, 80, 95, 90, 100].map((h, i) => (
              <div key={i} className="w-full flex flex-col items-center gap-2 group relative">
                <div className="w-full flex justify-center items-end gap-1 h-48">
                  {/* Users Bar */}
                  <div className="w-1/2 bg-gradient-to-t from-indigo-900 to-indigo-500 rounded-t-sm opacity-80 group-hover:opacity-100 transition-opacity" style={{ height: `${h}%` }}></div>
                  {/* Volume Bar */}
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="text-indigo-400 font-bold">12,450 actives</span>
                </div>
                <div className="w-full h-1.5 bg-slate-800 rounded-full"><div className="w-2/3 h-full bg-indigo-500 rounded-full"></div></div>
              </div>
              <div>
                <div className="flex justify-between text-xs mb-1">
          ...
          <div className="bg-[#101728] border border-slate-800/80 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button className="text-[10px] text-indigo-400 font-bold uppercase tracking-wider hover:text-indigo-300">Tout voir</button>
            </div>
