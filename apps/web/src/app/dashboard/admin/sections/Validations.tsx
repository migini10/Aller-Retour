  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-white flex items-center gap-2"><ShieldCheck className="w-5 h-5 text-indigo-400" /> Centre de Validations & Conformité</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[#101728] border border-slate-800/80 rounded-2xl overflow-hidden h-[600px] flex flex-col">
          ...
          <div className="overflow-y-auto p-4 space-y-3 flex-1">
            {validations.map((v, i) => (
              <div key={v.id} className={`p-4 rounded-xl border cursor-pointer transition-all ${i === 0 ? 'bg-indigo-500/10 border-indigo-500/30' : 'bg-slate-900/50 border-slate-800 hover:border-slate-600'}`}>
        ...
        <div className="bg-[#101728] border border-slate-800/80 rounded-2xl p-6 flex flex-col h-[600px]">
          <div className="flex justify-between items-start mb-6">
            <div>
              <span className="text-xs font-bold text-indigo-400 uppercase tracking-wider mb-1 block">Création GIE</span>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between bg-slate-800 p-2 rounded-lg">
                  <div className="flex items-center gap-2 text-sm text-white"><FileText className="w-4 h-4 text-indigo-400" /> Statuts_GIE.pdf</div>
                  <button className="text-indigo-400 hover:text-indigo-300 p-1"><Eye className="w-4 h-4" /></button>
                </div>
                <div className="flex items-center justify-between bg-slate-800 p-2 rounded-lg">
                  <div className="flex items-center gap-2 text-sm text-white"><FileText className="w-4 h-4 text-indigo-400" /> Certificat_NINEA.pdf</div>
                  <button className="text-indigo-400 hover:text-indigo-300 p-1"><Eye className="w-4 h-4" /></button>
                </div>
              </div>
            </div>
