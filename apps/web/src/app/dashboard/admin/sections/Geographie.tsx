  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {pays.map(p => (
          <div key={p.code} className="bg-[#141414] border border-[#2A2A2A]/80 hover:border-orange-500/30 rounded-3xl p-5 transition-colors relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-6">
              ...
              <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded border ${p.statut === 'Actif' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : p.statut === 'Beta' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-[#222222] text-slate-400 border-[#333333]'}`}>
                {p.statut}
              </span>
            </div>
            ...
            <div className="pt-4 border-t border-[#2A2A2A]/80 flex items-center justify-between">
              ...
              <button className="w-8 h-8 rounded-lg bg-[#1A1A1A] border border-[#333333] flex items-center justify-center text-slate-400 hover:text-white hover:bg-[#222222] transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-4">
            <div>
              ...
              <input type="text" value="18%" readOnly className="w-full bg-[#1A1A1A] border border-[#333333] rounded-xl px-4 py-2.5 text-sm text-slate-200 outline-none" />
            </div>
            <div>
              <label className="text-xs text-slate-500 font-semibold mb-1 block">Devise d'affichage</label>
              <input type="text" value="XOF (FCFA)" readOnly className="w-full bg-[#1A1A1A] border border-[#333333] rounded-xl px-4 py-2.5 text-sm text-slate-200 outline-none" />
            </div>
          </div>

          <div className="md:col-span-2 bg-[#1A1A1A]/50 border border-[#2A2A2A] rounded-2xl p-5">
            <p className="text-sm font-bold text-white mb-4">Fournisseurs de Paiement Actifs</p>
            <div className="space-y-3">
              <div className="flex items-center justify-between bg-[#222222]/50 p-3 rounded-xl border border-[#333333]">
