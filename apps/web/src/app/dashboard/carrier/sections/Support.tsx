...
export default function SectionSupport() {
  return (
    <div className="space-y-6">
      <h2 className="text-lg font-bold text-white flex items-center gap-2"><HelpCircle className="w-5 h-5 text-indigo-400" /> Support Premium GIE</h2>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-gradient-to-br from-indigo-900 to-indigo-600 rounded-3xl p-6 text-white border border-indigo-500/30">
          <MessageCircle className="w-8 h-8 mb-4 text-indigo-300" />
          ...
          <button className="bg-white text-indigo-900 font-bold px-4 py-2.5 rounded-xl text-sm w-full transition-colors hover:bg-slate-100">Démarrer le chat</button>
        </div>
      ...
      <div className="bg-[#101728] border border-slate-800 rounded-3xl p-6">
        <h3 className="font-bold text-white mb-6 flex items-center gap-2"><FileText className="w-4 h-4 text-indigo-400" /> Centre d'Aide & FAQ</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {['Comment assigner un chauffeur indépendant ?', 'Exporter les rapports comptables', 'Gérer les permissions de mon équipe', 'Intégration API pour réservations'].map((q, i) => (
            <div key={i} className="p-4 rounded-xl border border-slate-800/60 bg-slate-900/50 hover:bg-slate-800 hover:border-indigo-500/30 transition-colors cursor-pointer">
