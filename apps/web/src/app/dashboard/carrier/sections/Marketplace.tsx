        ...
        {chauffeursLibres.map(c => (
          <div key={c.id} className="bg-[#101728] border border-slate-800/80 hover:border-orange-500/30 rounded-2xl p-5 transition-colors">
            ...
            <div className="flex flex-wrap gap-2 mt-4 mb-5">
              <span className="bg-slate-900 border border-slate-700 px-2 py-1 rounded text-xs text-slate-300 flex items-center gap-1"><MapPin className="w-3 h-3" /> {c.distance}</span>
              <span className="bg-slate-900 border border-slate-700 px-2 py-1 rounded text-xs text-slate-300 flex items-center gap-1"><Star className="w-3 h-3 text-amber-400 fill-amber-400" /> {c.note}</span>
            </div>
