    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="bg-[#101728] border border-slate-800 rounded-2xl p-5">
          ...
          <button className="bg-slate-800 hover:bg-slate-700 text-white font-bold px-4 py-2 rounded-xl text-sm w-full transition-colors border border-slate-700">Appeler le 800 00 00 00</button>
        </div>
      </div>
      ...
      <div className="bg-[#101728] border border-slate-800 rounded-2xl p-5">
        ...
        <div className="space-y-3">
          {['Comment déclarer un incident pendant un trajet ?', 'Comment fonctionnent les retraits vers Wave ?', 'Que faire si un passager est absent ?'].map((q, i) => (
            <div key={i} className="p-4 rounded-xl border border-slate-800/60 bg-slate-900/50 hover:border-orange-500/30 transition-colors cursor-pointer">
