  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-orange-500/10 border border-orange-500/30 rounded-2xl p-4 flex items-start gap-4">
        ...
        <div>
          <p className="text-sm font-bold text-orange-400">Recherche Active</p>
          <p className="text-xs text-slate-300 mt-1">Vous recevrez des alertes pour les missions à moins de 15km de votre position actuelle (Dakar).</p>
        </div>
      </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  {m.urgent && <span className="bg-rose-500/20 text-rose-400 border border-rose-500/30 px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider">Urgent</span>}
                  <span className="text-xs text-slate-500 font-mono">Mission {m.id}</span>
                  <span className="text-xs text-slate-400">• {m.transporteur}</span>
                </div>
                <h3 className="text-lg font-bold text-white">{m.trajet}</h3>
