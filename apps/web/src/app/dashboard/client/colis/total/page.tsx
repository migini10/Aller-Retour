...
export default function ColisTotalPage() {
  ...
  return (
    <div className="min-h-screen pb-20 bg-slate-50 dark:bg-black transition-colors duration-300">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-24">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white flex items-center gap-3">
              <Package className="w-7 h-7 text-purple-500" /> Tous mes colis (Total)
            </h1>
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Historique complet de vos envois sur l'année.</p>
          </div>
        </div>
        ...
        <div className="mb-8 relative max-w-md">
          ...
          <input 
            ...
            className="w-full bg-white dark:bg-[#141414] border border-slate-200 dark:border-[#2A2A2A] rounded-xl pl-12 pr-4 py-3.5 text-slate-900 dark:text-white placeholder:text-slate-400 text-sm outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all shadow-sm"
          />
        </div>
        ...
        {loading ? (
          ...
          <div className="grid gap-4">
            {filteredColis.map((c, i) => (
              <div key={i} className="bg-white dark:bg-[#141414] border border-slate-200 dark:border-[#2A2A2A] rounded-2xl p-5 flex flex-col sm:flex-row justify-between sm:items-center gap-4 hover:border-purple-500/30 transition-colors shadow-sm">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider ${
                      c.statut === 'Livré' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' :
                      c.statut === 'En transit' || c.statut === 'Accepté' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400' :
